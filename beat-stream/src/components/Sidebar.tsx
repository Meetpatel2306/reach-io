...
import { Home, Search, Library, Settings, ListMusic, Heart, Headphones, Clock, Sparkles } from "lucide-react";
  ...
  return (
    <aside className="hidden md:flex flex-col w-60 lg:w-64 bg-black h-screen sticky top-0 p-4 gap-2 border-r border-white/5">
      ...
      <div className="mt-6 pt-4 border-t border-white/5 flex-1 overflow-y-auto">
        <div className="px-3 py-2 text-secondary text-xs font-semibold uppercase tracking-wider">Your Playlists</div>
        ...
        {userPlaylists.map((p) => (
          <Link key={p.id} href={`/playlist/user/${p.id}`} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-card text-secondary hover:text-white">
