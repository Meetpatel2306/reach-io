  ...
  return (
    <div className="bg-violet-500/5 border border-violet-500/20 rounded-xl p-3 sm:p-4 mb-4">
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        ...
        <span className="text-[11px] text-slate-500 w-full sm:w-auto">use {"{first_name}"} {"{company}"} {"{role}"}</span>
      </div>
      ...
      {showSaveModal && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-700 rounded-t-2xl sm:rounded-xl p-5 w-full max-w-md space-y-4 max-h-[90vh] overflow-y-auto pb-[max(1.25rem,env(safe-area-inset-bottom))]">
            <h3 className="text-white font-semibold text-lg">Save as template</h3>
            <div>
              <label className="text-xs text-slate-400 uppercase tracking-wider">Template name</label>
              <input className="input-field text-base sm:text-sm w-full mt-1 py-3 sm:py-2" placeholder="e.g. Python Backend Developer" value={newName} onChange={(e) => setNewName(e.target.value)} />
            </div>
            <div>
              <label className="text-xs text-slate-400 uppercase tracking-wider">Role type (auto-suggest)</label>
              <input className="input-field text-base sm:text-sm w-full mt-1 py-3 sm:py-2" placeholder="e.g. python backend developer" value={newRoleType} onChange={(e) => setNewRoleType(e.target.value)} />
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:justify-end pt-1">
              ...
              <button onClick={handleSave} disabled={saving} className="w-full sm:w-auto btn-primary py-3 sm:py-2 text-sm flex items-center justify-center gap-1.5 active:scale-[0.98] transition">
